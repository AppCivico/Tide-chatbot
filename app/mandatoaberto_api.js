...
module.exports = {
	...
	async getRecipient(user_id, recipient) {
		//const recipientData_qs = queryString.stringify(recipient);
		const res = await request.get(`${apiUri}/api/chatbot/recipient?security_token=${security_token}&`).query({ politician_id: user_id });
		const recipientData = await res.json();
		return recipientData;
	},
